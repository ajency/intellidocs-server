...
function dmt_menu_page_subscriber() { 
 ...
 function dmt_login_redirect_subscriber ($redirect_to, $request, $user) {
	 if (isset($user->roles))
	{
		...
		if(in_array('administrator',$user->roles) || in_array('dmt_site_admin',$user->roles)|| in_array('intellidocs_admin',$user->roles))
		{
...
//code added by Surekha : returns an files filtered by folders accessable to the logged in user///
...
function dmt_show_menu_page_move_multiple_files(){
	...
	<input type="hidden" name="cat" id="cat" value="<?php echo $_REQUEST['tag_ID'];?>" />
